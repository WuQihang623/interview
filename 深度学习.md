...
用于神经网络的可视化技术，特别是在图像分类任务中，用于理解模型对特定类别作出预测的依据。两者都能生成热力图，显示输入图像中哪些区域对模型预测某个类别最为重要。

**CAM：**

![image-20240709171401068](assets/image-20240709171401068.png)

要求网络最后要有卷积层和GAP，这样利用最后一层的卷积层进行加权求和，得到热力图。

**Grad CAM:** 

![在这里插入图片描述](https://img-blog.csdnimg.cn/67e80e7b700b4e28b1a03f64fabc74d7.png#pic_center)

将logits对应类别的梯度反向传播到卷积层，可以得到每个卷积层的梯度信息，求每个通道的梯度均值，再利用该均值与卷积层输出进行加权求和，再经过ReLU，即可得到热力图。

**区别：**

- 要使用CAM需要修改网络结构，所以需要重新训练
- Grad-CAM 通过计算梯度来加权特征图，而 CAM 直接使用网络中的类别权重
...
对于位置编码，常规的做法是在计算 query，key 和 value 向量之前，会计算一个位置编码向量  加到词嵌入上， 然后再乘以对应的变换矩阵
...
![image-20240708180824054](assets/image-20240708180824054.png)

$r_{ij}$为位置关系的偏置

**旋转位置编码：**

通过绝对位置编码的方式实现相对位置编码，综合了绝对位置编码和相对位置编码的优点。

对attention中的q, k向量注入了绝对位置信息，然后用更新的q,k向量做attention中的内积就会引入相对位置信息了。

![img](https://img-blog.csdnimg.cn/img_convert/273d137684a26ed345f10730628bc7c1.webp?x-oss-process=image/format,png)
...
### Transformer和Mamba的区别

**SSM模块**

![img](https://img-blog.csdnimg.cn/direct/e762176678fd49c7a99b7ad49602313c.png)

**Mamba结构：**

![img](https://img-blog.csdnimg.cn/direct/42661178d40b41fdab6582f2daa47798.png)

Mamba与Transformer相比，两者在训练时都能并行化，但Mamba在推理速度上更快。在推理时，Transformer的计算复杂度在输入序列长度增加时会显著上升，而Mamba的性能在这方面更为稳定。

Mamba模型在处理长序列时展现出明显优势，提供了更高的效率和竞争力。

Mamba计算效率高，既保留了RNN的形式又能转变为线性Transformer形式，使得推理速率提高。



### Transformer中残差结构的意义

在模型能够收敛的情况下，随着网络层数的增加，模型的性能（如准确率）并未得到提升，反而出现下降的现象。

为了解决网络退化问题，提出了采用残差结构。



### 为什么Transformer适合多模态任务

利用**自注意力机制**，允许模型在各个位置的输入之间进行任意形式的加权组合。这为多模态数据（如图像和文本）之间的关联关系提供了极大的灵活性，使得模型可以更好地捕捉各种模态之间的复杂交互和依赖。

Transformer 能够进行**高效的并行计算**，大大加速了模型的训练和推理速度，特别适合处理大量多模态数据。

通过**堆叠多层**的自注意力和前馈神经网络，Transformer 能够捕捉更深层次的语义和关系，特别适合复杂的多模态任务。

**多头自注意力**允许模型从不同的角度和尺度捕捉信息，为多模态任务中的多种模式和关系提供了广泛的视角。



### Transformer的并行化体现在哪里

Transformer的Encoder部分里，输出与之前所有的输入都有关，并不是只依赖上一个输入，因此，Transformer的Encoder可以并行化计算所有的自注意力机制。



### Transformer为什么要使用多头注意力机制

**多头可以学习到不同维度的特征和信息**。每一个self attention最终会产生一个维度上的输出特征，所以当使用多头注意力机制的时候，模型就可以学习到多维度的特征信息，这使得模型可以从多个维度更好的理解数据。



### Transformer训练的Dropout怎么设定

决定dropout之前，需要先判断是否模型过拟合。先dropout=0， 训练后得到模型的一些指标（比如: F1, Accuracy, AP）。比较train数据集和test数据集的指标。
...
2. 任意时刻的$x_t$可以又$x_0$和高斯分布方差$\beta$表示：

$$
x_t=\sqrt{\hat \sigma_t}x_0+\sqrt{1-\hat \sigma_t}\hat z_t
$$

其中，$\alpha_t=1-\beta_t$, $\hat \sigma_t=\sigma_t \sigma_{t-1}...\sigma_0$,$z_t$为标准高斯噪声。

**逆扩散过程：**



![img](https://pic1.zhimg.com/80/v2-65bf3a96fb73eda390b02fdf602f0750_720w.webp)
...
**Motivation:**为了**降低训练扩散模型的算力**，LDMs使用一个Autoencoder去**学习能尽量表达原始image space的低维空间表达**（latent embedding），这样可以大大减少需要的算力。

**Method:**

1. 感知压缩，感知压缩主要利用一个预训练的AutoEncoder（下图的左半边部分），该模型能够学习到一个在感知上等同于图像空间的潜在表示空间。这种方法的一个优势是只需要训练一个通用的自编码模型，就可以用于不同的扩散模型的训练，在不同的任务上使用。
2. 潜在扩散模型：一个时序去噪自编码器，根据输入预测一个对应去噪的噪音
3. 条件机制：通过在Unet主干网络上增加cross-attention机制来实现可控制的生成，同时引入一个专用编码器来处理各种形态的输入（文本，类别，layout等），其中Q为latent embedding，KV都为condition embedding计算而来

![image-20240425094648890](assets/image-20240425094648890.png)
